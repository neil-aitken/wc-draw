...
class TestTeamCityProbabilities:
    ...
    def test_scotland_city_probabilities(self):
        ...
        scotland_cities = calculate_team_city_probabilities(
            scotland_groups, 3, city_map
        )
        ...
        assert mexico_city_prob >= group_a_pct, \
            f"Mexico City prob ({mexico_city_prob}) should be >= Group A contribution ({group_a_pct})"

...
def check_inter_path_2_opponents():
        ...
        for line_num, line in enumerate(f, 1):
            ...
            for group_name, teams in draw_data['groups'].items():
                if 'Inter Path 2' in teams:
                    ...
                    has_african_opponent = any(
                        team in AFRICAN_TEAMS for team in other_teams
                    )
                    ...
                    european_count = sum(
                        1 for team in other_teams if team in EUROPEAN_TEAMS
                    )
                    ...
                    if european_count == 2:
                        draws_with_two_europeans.append({
                            ...
                            'teams': teams,
                            'european_teams': [
                                t for t in other_teams if t in EUROPEAN_TEAMS
                            ],
                        })
                    else:
                    ...
                    has_non_euro_pot1 = any(
                        team in NON_EUROPEAN_POT_1 for team in teams
                    )

                    if has_non_euro_pot1:
                        non_euro_pot1_team = [
                            t for t in teams if t in NON_EUROPEAN_POT_1
                        ][0]
    ...
    print("\n--- African Teams ---")
    print(
        f"Draws where Inter Path 2 faces an African team: "
        f"{len(draws_with_african_opponent):,}"
    )

    if draws_with_african_opponent:
        print(
            f"Percentage: "
            f"{len(draws_with_african_opponent) / total_draws * 100:.2f}%"
        )
        print("\nFirst 5 examples:")
        print("-" * 70)
        ...
        print(
            "Inter Path 2 never grouped with African teams in this dataset."
        )
    ...
    if draws_with_two_europeans:
        print(
            f"Percentage: "
            f"{len(draws_with_two_europeans) / total_draws * 100:.2f}%"
        )
        print("\nFirst 5 examples:")
        print("-" * 70)
    ...
    if draws_without_two_europeans:
        print(
            f"\nDraws with other European counts: "
            f"{len(draws_without_two_europeans):,}"
        )
        print(
            f"Percentage: "
            f"{len(draws_without_two_europeans) / total_draws * 100:.2f}%"
        )
        print("\nFirst 5 examples:")
        print("-" * 70)
    ...
    if draws_with_non_euro_pot1:
        print(
            f"Percentage: "
            f"{len(draws_with_non_euro_pot1) / total_draws * 100:.2f}%"
        )
        print("\nFirst 5 examples:")
        print("-" * 70)
        ...
        print(
            "Inter Path 2 never grouped with non-European Pot 1 teams."
        )
